    ...
    <Card className="border-purple-200">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center text-sm">
            <Sparkles className="h-4 w-4 mr-2 text-[#f3c012]" />
      ...
      {enabled && (
        <CardContent className="pt-0">
          <div className="space-y-3">
            {features.map((feature, index) => (
              <div key={index} className="flex items-start gap-3 p-2 bg-purple-25 rounded-lg">
                <div className="text-[#f3c012] mt-0.5">{feature.icon}</div>
          ...
          <Badge variant="outline" className="mt-3 text-xs text-[#f3c012] border-purple-200">
            Beta Feature
          </Badge>
        </CardContent>
      )}
